...
| J3 | P2 | candidate detail, portal, integrity | **Nine prose fields were sliced mid-word** — `rationale[:130]`, `span.text[:110]`, `question[:110]`, `answer[:400]`, `reason[:80]`, `detail[:120]` — rendering "lacks explicit a", "constitutes equivalent demonstrabl", "staff, d". Now word-boundary truncation with a visible ellipsis. The injected excerpt keeps a hard cut (it is forensic evidence, not prose) but gains the ellipsis so the reader can tell there is more. Found by reading the rendered page, not by any automated check |
